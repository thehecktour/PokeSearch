...
export default function Search() {
      ...
      try {
        ...
        const fetchPokemonInChunks = async (pokemonList) => {
          ...
          for (let i = 0; i < pokemonList.length; i += chunkSize) {
            ...
            allPokemon.push(...chunkResults.filter(pokemon => pokemon !== null));
            
            // Update progress
            const progress = Math.round((i + chunkSize) / pokemonList.length * 100);
  ...
  const filteredResults = pokemonList.filter((pokemon) => {
    ...
    const typeMatch = !filters.type || pokemon.types.some(t => t.type.name === filters.type);
    const weightMatch = (!filters.minWeight || pokemon.weight >= filters.minWeight) &&
                       (!filters.maxWeight || pokemon.weight <= filters.maxWeight);
    const heightMatch = (!filters.minHeight || pokemon.height >= filters.minHeight) &&
